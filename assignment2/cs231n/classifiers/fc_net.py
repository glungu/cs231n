...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          ...
          next_dim = hidden_dims[i] if i < self.num_layers - 1 else num_classes

          W = np.random.normal(0.0, weight_scale, (prev_dim, next_dim))
          b = np.zeros(next_dim)
          ...
          if self.use_batchnorm:
            if i < self.num_layers - 1:
              ...
              self.params[gamma_key] = np.float32(1.0) 
              self.params[beta_key] = np.float32(0.0)
        ...
        for i in range(self.num_layers):
          ...
          if (i < self.num_layers - 1):
            if self.use_batchnorm:
              ...
              a, cache = affine_batchnorm_relu_forward(a, W, b, gamma, beta, self.bn_params[i])
            else:
